...
- **Model provider**: The shipped `.claude/settings.json` does not select a provider, model, region, or reasoning effort. Claude Code keeps your existing subscription, API, Bedrock, or enterprise-gateway configuration. Run `aidlc config providers` to record and validate a project provider choice.
...
- **Settings**: `.claude/settings.json` pre-approves the framework's own commands (Read, Edit, Write, Glob, Grep, Task, WebSearch, the bun .claude/tools/ engine commands, and date -u) so workflows run without per-call permission prompts; a project's own build or test commands prompt once.
...
- **Document skill** (user-invocable): `.claude/skills/aidlc-knowledge/`, typed as `aidlc-knowledge`; the framework CLI also exposes `aidlc knowledge <verb>`. Also standalone — outside the lifecycle graph — but classified `read-write`, unlike the read-only session skills below: it changes the document catalog and emits document audit events. It never advances the workflow stage pointer and never approves a gate. See "Document knowledge" under "Where things live" in the project's root `AGENTS.md` (Claude Code and Copilot readers find it under "Shared AI-DLC onboarding" in this file).
- **Session skills** (read-only, user-invocable): `.claude/skills/aidlc-session-cost/`, `.claude/skills/aidlc-replay/`, `.claude/skills/aidlc-outcomes-pack/` — typed as `aidlc-session-cost`, `aidlc-replay`, `aidlc-outcomes-pack`. Each pulls every count from `aidlc engine runtime summary --json` (no LLM-side counting). Classified `read-only`: they never advance the workflow stage pointer and never emit audit events. `aidlc-session-cost` and `aidlc-replay` print to the terminal only; `aidlc-outcomes-pack` is the only one that writes a file (`OUTCOMES.md`).
